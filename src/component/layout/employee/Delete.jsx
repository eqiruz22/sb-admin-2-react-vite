  ...
  const handleDelete = (event) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`http://127.0.0.1:4000/employee/${isId}`, {
            method: "DELETE",
            headers: {
          ...
          if (res.ok) {
            await fetch(
              `http://127.0.0.1:4000/employee?page=${page}&size=${size}&query=${search}`,
              {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
              ...
              .then((res) => {
                ...
                setCountData(res.result.paginate["empCount"]);
              });
          }
          Swal.fire({
            title: "Deleted!",
